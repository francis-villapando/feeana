...
serve(async (req) => {
  try {
    ...
    const { joinCode } = body;
    if (!joinCode) return new Response(JSON.stringify({ error: "joinCode required" }), { status: 400 });
    ...
    const classRes = await fetch(`${supabaseUrl}/rest/v1/classes?join_code=eq.${encodeURIComponent(joinCode)}`, {
      headers: { Authorization: `Bearer ${serviceKey}`, apikey: serviceKey },
    });
    const classes = await classRes.json();
    if (!classRes.ok || classes.length === 0) return new Response(JSON.stringify({ error: "class not found" }), { status: 404 });
    const cls = classes[0];
