...
serve(async (req) => {
  try {
    ...
    if (!token || !content) return new Response(JSON.stringify({ error: "token and content required" }), { status: 400 });

    // derive sessionId from url
    const url = new URL(req.url);
    const parts = url.pathname.split("/");
    const sessionId = parts[parts.length - 2] === "session" ? parts[parts.length - 1] : parts[parts.length - 1];

    const tokenHash = hashToken(token, serverSalt);

    // find token row
    const findRes = await fetch(`${supabaseUrl}/rest/v1/submission_tokens?session_id=eq.${sessionId}&token_hash=eq.${tokenHash}&used=eq.false`, {
      headers: { Authorization: `Bearer ${serviceKey}`, apikey: serviceKey },
    });
    const rows = await findRes.json();
    if (!findRes.ok || rows.length === 0) return new Response(JSON.stringify({ error: "invalid_or_used_token" }), { status: 403 });
