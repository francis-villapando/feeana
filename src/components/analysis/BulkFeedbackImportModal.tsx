...
export function BulkFeedbackImportModal({
  ...
  const readFile = useCallback(async (file: File) => {
    if (!ACCEPTED_EXTENSIONS.some((ext) => file.name.toLowerCase().endsWith(ext))) {
      toast.error("Unsupported file type — use .csv, .tsv, or .txt");
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-lg overflow-hidden border border-border/80 bg-background/95 p-6 shadow-2xl backdrop-blur-xl sm:rounded-2xl">
        <DialogHeader>
        ...
        {mode === "file" ? (
          <div
            ...
            onDrop={handleDrop}
            className={cn(
              "mt-4 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed px-6 py-10 text-center transition-colors",
              dragging
        ...
        {fileName && mode === "file" && (
          <div className="mt-3 flex items-center gap-2 rounded-lg border border-border/60 bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
        ...
        {parsed && (
          <div className="mt-4 space-y-3">
            ...
            {parsed.items.length > 0 && (
              <ul className="space-y-2 rounded-lg border border-border/60 bg-muted/40 p-3">
