...
function enumerateLabels(labels: string[]): ReactNode {
  ...
  if (labels.length === 2)
    return (
      <>
        <AccentLabel>{labels[0]}</AccentLabel> and <AccentLabel>{labels[1]}</AccentLabel>
      </>
    );
...
function interpretAspectOrIssue(
  data: DistEntry[],
  totalFeedback: number,
  kind: "aspect" | "issue",
): ReactNode {
  ...
  const firstSentence =
    maxCount === 0 ? (
      ...
      <>
        Students most frequently {topicNoun} {enumerateLabels(topLabels)} ({maxCount}{" "}
        {maxCount === 1 ? "mention" : "mentions"}, {formatPercent(maxCount, totalFeedback)} of
        responses).
      </>
    );
  ...
  const secondSentence =
    missingLabels.length === 0 ? (
      ...
      <>
        {" "}
        No feedback was received about {enumerateLabels(missingLabels)}, which may indicate limited
        student engagement with {missingLabels.length === 1 ? "that area" : "those areas"}.
      </>
    );
...
function interpretPolarity(data: DistEntry[], totalFeedback: number): ReactNode {
  ...
  if (topEntries.length > 1) {
    ...
    const suffix = hasNegativeTie ? " — students raised concerns worth reviewing." : ".";
    return (
      <>
        Feedback was tied between {enumerateLabels(topLabels)} ({maxCount}{" "}
        {maxCount === 1 ? "mention" : "mentions"} each, {formatPercent(maxCount, totalFeedback)} of
        responses){suffix}
      </>
    );
  }
  ...
  if (topLabels[0] === "Negative") {
    return (
      <>
        The majority of feedback was <AccentLabel>Negative</AccentLabel> ({maxCount}{" "}
        {maxCount === 1 ? "mention" : "mentions"}, {formatPercent(maxCount, totalFeedback)} of
        responses) — students raised concerns worth reviewing.
      </>
    );
  }
  ...
  const firstSentence = (
    <>
      The majority of feedback was <AccentLabel>{topLabels[0]}</AccentLabel> ({maxCount}{" "}
      {maxCount === 1 ? "mention" : "mentions"}, {formatPercent(maxCount, totalFeedback)} of
      responses).
    </>
  );
  ...
  const secondSentence =
    negativePercent > 30 ? (
      <>
        {" "}
        However, {negativeCount} responses ({negativePercent}%) were negative — students raised
        significant concerns worth reviewing.
      </>
    ) : (
      ""
    );
...
function interpretRbt(data: DistEntry[], totalFeedback: number): ReactNode {
  ...
  const firstSentence =
    maxCount === 0 ? (
      ...
      <>
        Student feedback primarily reflects {enumerateLabels(topLevelLabels)} ({maxCount}{" "}
        {maxCount === 1 ? "mention" : "mentions"}, {formatPercent(maxCount, totalFeedback)} of
        responses).
      </>
    );
...
function interpretClt(data: DistEntry[], totalFeedback: number): ReactNode {
  ...
  const allTied =
    categorizedEntries.length > 1 &&
    categorizedEntries.every((entry) => entry.value === categorizedEntries[0].value);
  ...
  if (maxCount === 0) {
    ...
    firstSentence = (
      <>
        Feedback indicates <AccentLabel>Intrinsic</AccentLabel> and{" "}
        <AccentLabel>Extraneous</AccentLabel> load are equally prominent ({maxCount}{" "}
        {maxCount === 1 ? "mention" : "mentions"} each, {formatPercent(maxCount, totalFeedback)} of
        responses).
      </>
    );
    ...
    firstSentence = (
      <>
        Feedback indicates <AccentLabel>{topLabels[0]}</AccentLabel> load is the primary cognitive
        burden ({maxCount} {maxCount === 1 ? "mention" : "mentions"},{" "}
        {formatPercent(maxCount, totalFeedback)} of responses).
      </>
    );
  }
